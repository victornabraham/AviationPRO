		...
		onTheMasterPage: {
			...
			actions: {
									...
									var sCurrentId,
										aItemsNotInTheList = aEntityData.filter(function (oObject) {
											return !oList.getItems().some(function (oListItem) {
												return oListItem.getBindingContext().getProperty("TravelID") === oObject.TravelID;
											});
										});

									if (!aItemsNotInTheList.length) {
										// Not enough items all of them are displayed so we take the last one
										sCurrentId = aEntityData[aEntityData.length - 1].TravelID;
									} else {
										sCurrentId = aItemsNotInTheList[0].TravelID;
									}
				...
				iRememberTheListItem: function (oListItem) {
					...
					this.getContext().currentItem = {
						...
						id: oBindingContext.getProperty("TravelID"),
						title: oBindingContext.getProperty("AgencyName")
					};
				},
			...
			assertions: {
				...
				theListHeaderDisplaysZeroHits: function () {
					return this.waitFor({
						...
						matchers: new PropertyStrictEquals({
							name: "title",
							value: "TravelProcessor (0)"
						}),
						success: function () {
							Opa5.assert.ok(true, "The list header displays 'TravelProcessor (0)'");
						},
				...
				theListShouldBeSortedAscendingOnUnitNumber: function () {
					return this.theListShouldBeSortedAscendingOnField("TotalPrice");
				},

				theListShouldBeSortedAscendingOnName: function () {
					return this.theListShouldBeSortedAscendingOnField("AgencyName");
				},
				...
				theListShouldHaveAllEntries: function () {
					...
					// retrieve all TravelProcessor to be able to check for the total amount
					...
					return this.waitFor({
						...
						success: function (oList) {
							...
							this.waitFor({
								...
								matchers: new PropertyStrictEquals({
									name: "title",
									value: "TravelProcessor (" + iExpectedLength + ")"
								}),
								success: function () {
