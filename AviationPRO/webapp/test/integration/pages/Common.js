sap.ui.define([
	...
	function getFrameUrl(sHash, sUrlParameters) {
		var sUrl = jQuery.sap.getResourcePath("AviationUI/AviationPRO/app", ".html");
		sHash = sHash || "";
		sUrlParameters = sUrlParameters ? "?" + sUrlParameters : "";

		if (sHash) {
			sHash = "#AviationPROUI-display&/" + (sHash.indexOf("/") === 0 ? sHash.substring(1) : sHash);
		} else {
			sHash = "#AviationPROUI-display";
		}

		return sUrl + sUrlParameters + sHash;
	}

	return Opa5.extend("AviationUI.AviationPRO.test.integration.pages.Common", {
		...
		getMockServer: function () {
			return new Promise(function (success) {
				Opa5.getWindow().sap.ui.require(["AviationUI/AviationPRO/localService/mockserver"], function (mockserver) {
