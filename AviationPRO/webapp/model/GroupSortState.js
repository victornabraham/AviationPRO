sap.ui.define([
	...
	return BaseObject.extend("AviationUI.AviationPRO.model.GroupSortState", {
		 ...
		 * @class
		 * @public
		 * @alias AviationUI.AviationPRO.model.GroupSortState
		 */
		...
		/**
		 * Sorts by AgencyName, or by TotalPrice
		 *
		...
		/**
		 * Groups by TotalPrice, or resets the grouping for the key "None"
		 *
		 * @param sKey - the key of the field used for grouping
		 * @returns {sap.ui.model.Sorter[]} an array of sorters
		 */
		group: function (sKey) {
			...
			if (sKey === "TotalPrice") {
				...
				this._oViewModel.setProperty("/sortBy", "TotalPrice");

				aSorters.push(
					new Sorter("TotalPrice", false,
						this._fnGroupFunction.bind(this))
				);
			} else if (sKey === "None") {
				// select the default sorting again
				this._oViewModel.setProperty("/sortBy", "AgencyName");
			}
