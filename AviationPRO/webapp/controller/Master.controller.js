/*global history */
sap.ui.define([
	"AviationUI/AviationPRO/controller/BaseController",
	"sap/ui/model/json/JSONModel",
	"sap/ui/model/Filter",
	"sap/ui/model/FilterOperator",
	"sap/m/GroupHeaderListItem",
	"sap/ui/Device",
	"AviationUI/AviationPRO/model/formatter",
	"sap/m/MessageBox",
	"AviationUI/AviationPRO/model/grouper",
	...
	return BaseController.extend("AviationUI.AviationPRO.controller.Master", {
		...
		onSearch: function (oEvent) {
			...
			if (sQuery) {
				this._oListFilterState.aSearch = [new Filter("AgencyName", FilterOperator.Contains, sQuery)];
			} else {
		...
		onOpenViewSettings: function () {
			if (!this._oViewSettingsDialog) {
				this._oViewSettingsDialog = sap.ui.xmlfragment("AviationUI.AviationPRO.view.ViewSettingsDialog", this);
		...
		onConfirmViewSettingsDialog: function (oEvent) {
			...
			aFilterItems.forEach(function (oItem) {
				switch (oItem.getKey()) {
				case "Filter1":
					aFilters.push(new Filter("TotalPrice", FilterOperator.LE, 100));
					break;
				case "Filter2":
					aFilters.push(new Filter("TotalPrice", FilterOperator.GT, 100));
					break;
				default:
					break;
				}
				...
				function (mParams) {
					...
					if (!mParams.list.getSelectedItem()) {
						this.getRouter().navTo("object", {
							TravelID: encodeURIComponent(mParams.firstListitem.getBindingContext().getProperty("TravelID"))
						}, true);
					}
				}.bind(this),
				function (mParams) {
		...
		_showDetail: function (oItem) {
			...
			this.getRouter().navTo("object", {
				TravelID: encodeURIComponent(oItem.getBindingContext().getProperty("TravelID"))
			}, bReplace);
		},
